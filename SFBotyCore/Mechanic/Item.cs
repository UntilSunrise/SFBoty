...
namespace SFBotyCore.Mechanic {
	public class Item {
		...
		public Item(string[] responseString, int offset, int offsetItemIDMagicShop) {
			...
			enchantment = Convert.ToInt32(typeOriginal / Math.Pow(2, 24));
			socket = Convert.ToInt32(typeOriginal - (enchantment * Math.Pow(2, 24)));
			socket = Convert.ToInt32(socket / Math.Pow(2, 16));
			typeOriginal = ((typeOriginal - (enchantment * Math.Pow(2, 24))) - (socket * Math.Pow(2, 16)));
			enchantmentPower = Convert.ToInt32(picOriginal / Math.Pow(2, 16));
			picOriginal = (picOriginal - (enchantmentPower * Math.Pow(2, 16)));
			socketPower = Convert.ToInt32(mushroomValue / Math.Pow(2, 16));
